...
struct KdTree
{
	...
	void insertNode(Node **node, uint depth, std::vector<double> point, int id) {
		if (*node == NULL) {
...
/************************************************/
	void searchForKDTreeNeighbors(std::vector<int> &ids, std::vector<double> target,
				double distanceTol, Node* parent_node, int depth) {

		if (parent_node != NULL) {
			double delta_x = parent_node->point[0] - target[0];
			double delta_y = parent_node->point[1] - target[1];
